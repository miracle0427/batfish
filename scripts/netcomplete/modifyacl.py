	...
	for line in file:
		...
		if m:
			...
			if not(os.path.isfile(srcfilename) and os.path.isfile(dstfilename)):
				print "src " + srcfilename + " or dst " + dstfilename + " file doesn't exist"
			else:
				pol = str(polnum)
				...
				interface = "access-list 1 permit any\n!\ninterface Loopback"+pol+"\n ip address 60." + pol2num + "." + pol3num + ".0 255.255.255.0\n ip access-group 1 in\n!\n"
				#print interface
				#print network
				srcfile.close()
				...
				interface = "access-list 1 permit any\n!\ninterface Loopback"+pol+"\n ip address 80.0.100"  + ".0 255.255.255.0\n ip access-group 1 in\n!\n"
				#print interface
				#print network
				dstfile.close()
				if exists:
	...
	for line in file:
		...
		if m:
			...
			if not(os.path.isfile(srcfilename) and os.path.isfile(dstfilename)):
				...
				network = " network 80.0.100" + ".0 mask 255.255.255.0\n"
				#print interface
				#print network
				dstfile.close()
