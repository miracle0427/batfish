...
TESTRIG_DIR=/users/anubhavnidhi/aceso/batfish/test_rigs/bbn
ORIG_CONFIGS_DIR=$TESTRIG_DIR/unbroken/configs
#BROKEN_CONFIGS_DIR=$TESTRIG_DIR/rm-network_half/configs
BROKEN_CONFIGS_DIR=$TESTRIG_DIR/broken/configs
...
echo "$BREAK_DEVICES" | while read DEVICE; do
    ...
    sed -e 's/network 1.*\n//' $ORIG_CONFIGS_DIR/$DEVICE > $BROKEN_CONFIGS_DIR/$DEVICE
done
